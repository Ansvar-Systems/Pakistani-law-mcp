/**
 * search_eu_implementations — Search EU directives/regulations referenced by Pakistani legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_pakistani_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  pakistani_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as pakistani_statute_count
  ...
  if (input.has_pakistani_implementation) {
    sql += ' HAVING pakistani_statute_count > 0';
  }
