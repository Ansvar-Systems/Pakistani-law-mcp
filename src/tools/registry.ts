/**
 * Tool registry for Pakistani Law MCP Server.
...
import { getPakistaniImplementations, type GetPakistaniImplementationsInput } from './get-pakistani-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Gazette of Pakistan. ' +
    ...
    'Call this FIRST when you need to understand what Pakistani legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Pakistani statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    ...
    description:
      'Validate a Pakistani legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Pakistani statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Pakistani?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Pakistani legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Pakistani statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Pakistani statute references or aligns with. ' +
      'Pakistani laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Pakistani statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_pakistani_implementations',
    description:
      'Find all Pakistani statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Pakistani statutes. ' +
      'Pakistani may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Pakistani legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_pakistani_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Pakistani legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Pakistani statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Pakistani statute or provision. ' +
      ...
      'Pakistani is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Pakistani statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_pakistani_implementations':
          result = await getPakistaniImplementations(db, args as unknown as GetPakistaniImplementationsInput);
          break;
