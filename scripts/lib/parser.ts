/**
 * HTML parser for Pakistani legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Pakistani legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Pakistani statute.
 *
 ...
 *   </div>
 */
export function parsePakistaniHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Pakistani acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Pakistani legal text.
 *
 * Pakistani definitions typically use patterns like:
