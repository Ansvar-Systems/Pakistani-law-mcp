#!/usr/bin/env tsx
/**
 * Pakistani Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://www.pakistancode.gov.pk';
const PORTAL_NAME = 'Pakistan Code';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/pakistani-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Pakistani Law MCP — Data Freshness Check');
